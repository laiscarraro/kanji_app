...
def test_from_login(test_user_builder):
    ...
    test_user_builder.from_login('test_user')
    login_after = test_user_builder.login
    assert (
        login_before is None
        and login_after == 'test_user'
    )

def test_set_user_information(test_user_builder):
    test_user_builder.from_login('test_user').set_user_information()
    assert test_user_builder.user_information.user_login.values[0] == 'test_user'
...
def test_get_anime_ids(test_user_builder):
    user = test_user_builder.from_login('test_user')
...
def test_build(test_user_builder):
    user = test_user_builder.from_login('test_user').build()
    assert (
        isinstance(user, User) and
        user.get_login() == 'test_user'
    )
...
def test_build_with_no_animes(test_user_builder):
    user = test_user_builder.from_login('test_user_no_animes').build()
    assert (
