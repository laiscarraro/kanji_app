...
from src.utils import color_target_kanji, get_anime_info, get_audio, get_translation_html
...
if st.session_state['user_found']:
    ...
    with c1:
        ...
        if kanji not in user.get_kanji():
            if st.button('Marcar como concluído'):
                user.update_kanji(
                    user.get_kanji() + kanji
                )
                st.balloons()
                st.success('Kanji adicionado com sucesso!')
        else:
